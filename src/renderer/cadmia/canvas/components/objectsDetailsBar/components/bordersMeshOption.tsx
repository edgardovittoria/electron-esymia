...
export const BordersMeshOption: React.FC<BordersMeshOptionProps> = () => {
    ...
    return (
        <div className="flex flex-row items-center gap-2 px-2 py-1">
            ...
            <label className={`text-xs font-medium cursor-pointer select-none ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'}`} onClick={() => (isBordersVisible) ? dispatch(disableBordersForComponent(componentKey)) : dispatch(enableBordersForComponent(componentKey))}>
                Show Borders
            </label>
        </div>
    )
