...
export const MaterialSelection: FC<MaterialSelectionProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2 mt-4">
      <h6 className={`text-xs font-bold uppercase tracking-wider ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>Material</h6>
      <div className="flex flex-row items-center gap-2">
        ...
        >
          <div className="relative w-full">
            ...
            >
              <Listbox.Options
                className={`absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm scrollbar-thin ${theme === 'light' ? 'bg-white scrollbar-thumb-gray-300' : 'bg-gray-800 scrollbar-thumb-gray-600'}`}
              >
                {user ? (
                    ...
                    <Listbox.Option
                      ...
                      className={({ active }) =>
                        `relative cursor-default select-none py-2 pl-10 pr-4 ${active
                          ? `${theme === 'light' ? 'bg-blue-100 text-blue-900' : 'bg-blue-900/30 text-blue-100'}`
                    ...
                    >
                      {({ selected }) => (
                        <>
                          ...
                          {selected ? (
                            <span className={`absolute inset-y-0 left-0 flex items-center pl-3 ${theme === 'light' ? 'text-blue-600' : 'text-blue-400'}`}>
                  ...
                  <Listbox.Option value={undefined} disabled>
                    <span className={`p-2 text-xs italic ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}>
        ...
        <div className="tooltip tooltip-left" data-tip="Material Details">
          <TbInfoSquareRounded
            className={`hover:cursor-pointer w-5 h-5 transition-colors ${theme === 'light' ? 'text-gray-500 hover:text-gray-700' : 'text-gray-400 hover:text-gray-200'}`}
            onClick={() => {
