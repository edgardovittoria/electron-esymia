...
const SimulationStatus: React.FC<SimulationStatusProps> = ({
  ...
  return (
    <div
      className={`absolute right-10 w-1/4 bottom-16 flex flex-col justify-center items-center bg-white p-3 rounded ${!feedbackSimulationVisible && 'hidden'}`}>
      <div className='flex flex-row justify-between'>
        <h5>Simulation Status</h5>
        <TiArrowMinimise className='absolute top-2 right-2 hover:cursor-pointer hover:bg-gray-200' size={20}
      ...
      <div className='mx-auto w-full max-w-md rounded-2xl bg-white p-2'>
        <Disclosure>
          {({ open }) => (
            <>
              <Disclosure.Button
                className='flex w-full justify-between rounded-lg border border-secondaryColor px-4 py-2 text-left text-sm font-medium text-secondaryColor hover:bg-green-100 focus:outline-none focus-visible:ring focus-visible:ring-green-500/75'>
                <span>{name}</span>
                <MdKeyboardArrowUp
                  className={`${
