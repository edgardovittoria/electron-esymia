import { crea_regioni } from "./crea_regioni";
import { genera_mesh } from "./genera_mesh";
import { MaterialMesher } from "./interfaces";
import { Material } from "../../renderer/cad_library/components/model/componentEntity/componentEntity";


export function creaBricks(
  bricksMatrix: number[][],
  bricksMaterials: number[],
): { bricks: number[][]; bricks_material: number[] } {
  const bricks: number[][] = [];
  const bricks_material: number[] = [];

  for (let i = 0; i < bricksMatrix.length; i++) {
    bricks.push(bricksMatrix[i]);
    bricks_material.push(bricksMaterials[i]);
  }

  return { bricks, bricks_material };
}
...
export function computeMeshV2(
  ...
  const { bricks, bricks_material } = creaBricks(bricksMatrix, bricksMaterials);
  let materialsMesher: MaterialMesher[] = materials.map((m) => {
    return {
      name: m.name,
      sigmar: m.conductivity,
...
self.onmessage = (ev: MessageEvent) => {
  const [den, freq_max, bricksS3, projectID] = ev.data
