...
import {
  getFolderByUserEmailDynamoDB,
  getSimulationProjectsByUserEmailDynamoDB,
...
export const useStorage = () => {
  ...
  const loadProjectsOnline = (setLoginSpinner: (v: boolean) => void) => {
    execQuery2(getFolderByUserEmailDynamoDB, user.email, dispatch).then(
      (resFolders) => {
        let folders = resFolders.Items.map(
          (f: any) =>
            ({
              id: f.id.S,
              folder: convertFromDynamoDBFormat(f),
            }) as DynamoFolder,
        );
        try {
          execQuery2(
            ...
            if (resProjects && resProjects.Items) {
              let projects = resProjects.Items.map(
                (p: any) =>
                  ({
                    id: p.id.S,
                    project: convertFromDynamoDBFormat(p),
                  }) as DynamoProject,
              );
              const rootFaunaFolder = {
                ...
                folder: {
                  ...
                  projectList: projects
                    .filter(
                      (p: { project: { parentFolder: string } }) =>
                        p.project.parentFolder === 'root',
                    )
                    .map((pr: { id: any }) => pr.id),
                  parent: 'nobody',
                },
