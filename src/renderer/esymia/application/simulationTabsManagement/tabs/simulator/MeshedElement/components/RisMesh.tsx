    ...
    <>
      {externalGrids.vertices.map((squareVertices, index) => (
        <RisMeshElement
          ...
          materialColor={
            materialList.filter(
              (m) => m.name === externalGrids.materials[index],
            )[0].color
          }
          positions={squareVertices}
...
export const InstancedSquaresFromVertices: FC<{
  externalGrids: ExternalGridsRisObject;
  resetFocus: Function;
}> = ({ externalGrids, resetFocus }) => {
  const selectedProject = useSelector(selectedProjectSelector) as Project;
  ...
  useEffect(() => {
    ...
    squares.forEach((vertices, i) => {
      ...
      const color = new THREE.Color(
        materialList.filter(
          (m) => m.name === externalGrids.materials[i],
        )[0].color,
      ); // Default: bianco
      ...
      if (Math.abs(normal.x) > 0) {
        scale.set(
          width * scalingViewParams.y,
          height * scalingViewParams.z,
          1 * scalingViewParams.x,
        );
      } else if (Math.abs(normal.y) > 0) {
        scale.set(
          width * scalingViewParams.x,
          height * scalingViewParams.z,
          1 * scalingViewParams.y,
        );
      } else {
        scale.set(
          width * scalingViewParams.y,
          height * scalingViewParams.x,
          1 * scalingViewParams.z,
        );
      }
      ...
      centroide.x = centroide.x * scalingViewParams.x;
      centroide.y = centroide.y * scalingViewParams.y;
      centroide.z = centroide.z * scalingViewParams.z;
